...
import Mapa from "./Map"; // Importar el componente de Mapa
...
const Cotizador = () => {
  ...
  const calcularRuta = async (latInmueble: number, lngInmueble: number) => {
    try {
      ...
      const response = await axios.get(`https://router.project-osrm.org/route/v1/driving/${oficinaAvaluador.lng},${oficinaAvaluador.lat};${lngInmueble},${latInmueble}?overview=false&steps=true`);
  ...
  return (
    <div className="max-w-3xl mx-auto p-8 bg-white shadow-lg rounded-xl mt-12">
      <h1 className="text-3xl font-bold text-center text-teal-600 mb-8">Cotizador de Avaluos</h1>

      {/* Selección de área */}
      <div className="mb-6">
        <label htmlFor="area" className="block text-lg font-semibold text-gray-700">Área (m²):</label>
        <select
          id="area"
          value={area}
          onChange={(e) => setArea(Number(e.target.value) as 20 | 40 | 60 | 80 | 100 | 120)}
      ...
      <div className="mb-6">
        <label htmlFor="pisos" className="block text-lg font-semibold text-gray-700">Número de pisos:</label>
        <input
          type="number"
          id="pisos"
          value={pisos}
      ...
      {distanciaRuta > 0 && (
        <div className="mt-6">
          <p className="text-lg text-gray-800">Distancia a la ubicación seleccionada: <strong>{distanciaRuta.toFixed(2)} km</strong></p>
          <p className="text-lg text-gray-800">Tiempo estimado de la ruta: <strong>{tiempoRuta}</strong></p>
        </div>
      )}

      {/* Mostrar el precio final */}
      <div className="mt-6 text-center">
        <h2 className="text-2xl font-semibold text-gray-800">Precio Final del Avaluo:</h2>
        <p className="text-3xl font-bold text-teal-600">{formatearPrecio(precioFinal)}</p>
      </div>
    </div>
  );
};
